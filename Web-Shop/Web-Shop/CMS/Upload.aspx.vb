...
Public Class Upload
    ...
    Protected Sub Page_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        Dim ArtikalID As Integer = Request.QueryString("id")
        Dim Tag As String = Request.QueryString("tag")
        umetniSliku(ArtikalID, Tag)
        Response.Redirect("Artikal.aspx?id=" & ArtikalID)
    End Sub

    Private Sub umetniSliku(ArtikalID As Integer, Tag As String)

        If Request.Files.Count > 0 Then
            'Dim zadanaSirina = "100" 'ConfigurationManager.AppSettings("ThumbWidth")
            Dim zadaniThumb = "1024" 'ConfigurationManager.AppSettings("NovaSirinaSlike")
            Dim i As Integer
            Dim datoteke As New List(Of Datoteka)()
            Dim folderThumb As String = "/Datoteke/Artikli/Thumb/"
            Dim folderPreview As String = "/Datoteke/Artikli/"
            ...
            For i = 0 To Request.Files.Count - 1
                ...
                Dim naziv As String = OcistiNazivDatoteke(datoteka.FileName, ArtikalID)
    ...
    Private Sub SpremuuBazu(naziv As String, ArtikalID As Integer, Tag As String)
        ...
        InsertKomanda.CommandText = "UPDATE ArtikliSlike SET Zadana='0' WHERE ArtikalID=" & ArtikalID & "; INSERT INTO ArtikliSlike (Datoteka, ArtikalID, Zadana) Values ('" & naziv & "', " & ArtikalID & ", '1'); INSERT INTO BazaSlika (Datoteka, MetaTag) Values ('" & naziv & "', '" & Tag & "')"
        konekcija.Open()
        InsertKomanda.ExecuteNonQuery()
        konekcija.Close()
    End Sub

    Private Function OcistiNazivDatoteke(naziv As String, ArtikalID As Integer) As String
        naziv = naziv.ToLower()
    ...
    Private Shared Function Thumb(image As Image, sirina As Integer, visina As Integer, putanja As String, selekcija As Rectangle) As Size
        ...
        Using bitmap As New Bitmap(sirina, visina)
            Using graphics As Graphics = graphics.FromImage(bitmap)
                ...
                If selekcija.Width > 0 Then
                    putanja = putanja.Replace("/Datoteke/Novosti/", String.Format("/Datoteke/Novosti/{0}x{1}", sirina, visina))
